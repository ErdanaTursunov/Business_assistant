  ...
  async respond(req, res) {
    try {
      ...
      if (Array.isArray(searchResult) && searchResult.length > 0) {
        ...
        if (score <= 0.55) {
          finalAnswer += `Өкінішке орай, бұл мәселені менеджерге жазып ғана шеше аласыз. Менеджер есімі Ақерке
...
Егер ақылы консультация алғыңыз келсе құны 5000, төлем жасап тікелей өзіме звандай берсеңіз болады!`;
  ...
  async process(req, res) {
    try {
      ...
      const history = await Message.findOne({
      ...
      if (!history) {
        ...
        await Message.create({
          ...
          message: question,
          ai_response: `Бұл автоматты ИИ асистент жауабы. Егер сізге Айдана Асқарқызы көмегі қажет болса немесе консультация әзірге менеджерге жазыңыз:
          Менеджер есімі Ақерке
          WhatsApp: +7 747 724 07 99
        ...
        return res.json({
          response: `Бұл автоматты ИИ асистент жауабы. Егер сізге Айдана Асқарқызы көмегі қажет болса немесе консультация әзірге менеджерге жазыңыз:
        Менеджер есімі Ақерке
        WhatsApp: +7 747 724 07 99
