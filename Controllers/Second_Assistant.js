...
class Second_Assistant {
  ...
  async getAIResponse(phoneNumber, userMessage) {
    try {
      ...
      - Жауаптарыңызды қысқа, бірақ нақты беріңіз.  до  этого был   так : const systemMessage = {
  role: "system",
  content: Сіз - Қазақстандағы заңгерлік қызметтерді сатуға арналған виртуалды көмекші.
...
@@ -71,44 +71,43 @@
  ...
  - Қажет болса, пайдаланушыға келесі қадамдарды түсіндіріңіз (мысалы, "Қызметті алу үшін сізге келесі қадамдарды орындау қажет...").
  
  ❌ Егер база ақпаратты тапса, бірақ ол пайдаланушының сұранысына жауап болмайтын болса:
  - "База бойынша табылған ақпарат сіздің сұранысыңызға нақты жауап болмауы мүмкін. Сіз нақты нені білгіңіз келеді?"
  ...
  📝 Жауап үлгілері:
  ✅ Егер қызмет табылса:
  "Сәлеметсіз бе! Біз ипотекалық сүйемелдеу қызметін ұсынамыз. Бұл қызметке мыналар кіреді: [қызмет сипаттамасы]. Сізге осы қызмет қажет пе?"
